    ...
    useEffect(() => {
        if (gig) {
            setFormData({
             ...
             setFormData({ name: '', supplierName: '', paymentAmount: 0, eventDate: today, paymentDueDate: today, invoiceNumber: '', notes: '', attachments: [], duration: undefined, summary: '', billingType: 'per_event' });
        }
    }, [gig]);
...
export const PackageFormModal: React.FC<PackageFormModalProps> = ({ packageItem, onClose, onSave }) => {
    ...
    return (
        <Modal title={packageItem ? 'עריכת חבילה' : 'חבילה חדשה'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4">
                 <div>
